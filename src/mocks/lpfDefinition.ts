import { COPAARGENTINA, COPADELALIGA_ARG, COPASUDAMERICANA, LPF_ARG } from "@/config"

export const LPF_STANDINGS = {
  "get": "standings",
  "parameters": {
    "league": "128",
    "season": "2024"
  },
  "errors": [],
  "results": 1,
  "paging": {
    "current": 1,
    "total": 1
  },
  "response": [
    {
      "league": {
        "id": 128,
        "name": "Liga Profesional Argentina",
        "country": "Argentina",
        "logo": "https:\/\/media.api-sports.io\/football\/leagues\/128.png",
        "flag": "https:\/\/media.api-sports.io\/flags\/ar.svg",
        "season": 2024,
        "standings": [
          [
            {
              "rank": 1,
              "team": {
                "id": 438,
                "name": "Velez Sarsfield",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/438.png"
              },
              "points": 48,
              "goalsDiff": 20,
              "group": "Liga Profesional",
              "form": "LWDDD",
              "status": "same",
              "description": "Copa Libertadores",
              "all": {
                "played": 26,
                "win": 13,
                "draw": 9,
                "lose": 4,
                "goals": {
                  "for": 36,
                  "against": 16
                }
              },
              "home": {
                "played": 12,
                "win": 9,
                "draw": 3,
                "lose": 0,
                "goals": {
                  "for": 19,
                  "against": 2
                }
              },
              "away": {
                "played": 14,
                "win": 4,
                "draw": 6,
                "lose": 4,
                "goals": {
                  "for": 17,
                  "against": 14
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 2,
              "team": {
                "id": 456,
                "name": "Talleres Cordoba",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/456.png"
              },
              "points": 46,
              "goalsDiff": 8,
              "group": "Liga Profesional",
              "form": "WWWWL",
              "status": "same",
              "description": null,
              "all": {
                "played": 26,
                "win": 12,
                "draw": 10,
                "lose": 4,
                "goals": {
                  "for": 32,
                  "against": 24
                }
              },
              "home": {
                "played": 12,
                "win": 7,
                "draw": 4,
                "lose": 1,
                "goals": {
                  "for": 16,
                  "against": 7
                }
              },
              "away": {
                "played": 14,
                "win": 5,
                "draw": 6,
                "lose": 3,
                "goals": {
                  "for": 16,
                  "against": 17
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 3,
              "team": {
                "id": 436,
                "name": "Racing Club",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/436.png"
              },
              "points": 43,
              "goalsDiff": 11,
              "group": "Liga Profesional",
              "form": "LLWWW",
              "status": "same",
              "description": null,
              "all": {
                "played": 26,
                "win": 13,
                "draw": 4,
                "lose": 9,
                "goals": {
                  "for": 41,
                  "against": 30
                }
              },
              "home": {
                "played": 12,
                "win": 8,
                "draw": 2,
                "lose": 2,
                "goals": {
                  "for": 24,
                  "against": 13
                }
              },
              "away": {
                "played": 14,
                "win": 5,
                "draw": 2,
                "lose": 7,
                "goals": {
                  "for": 17,
                  "against": 17
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 4,
              "team": {
                "id": 445,
                "name": "Huracan",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/445.png"
              },
              "points": 43,
              "goalsDiff": 11,
              "group": "Liga Profesional",
              "form": "LDLWW",
              "status": "same",
              "description": null,
              "all": {
                "played": 25,
                "win": 11,
                "draw": 10,
                "lose": 4,
                "goals": {
                  "for": 27,
                  "against": 16
                }
              },
              "home": {
                "played": 13,
                "win": 7,
                "draw": 5,
                "lose": 1,
                "goals": {
                  "for": 13,
                  "against": 4
                }
              },
              "away": {
                "played": 12,
                "win": 4,
                "draw": 5,
                "lose": 3,
                "goals": {
                  "for": 14,
                  "against": 12
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 5,
              "team": {
                "id": 435,
                "name": "River Plate",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/435.png"
              },
              "points": 40,
              "goalsDiff": 14,
              "group": "Liga Profesional",
              "form": "DWLWW",
              "status": "same",
              "description": null,
              "all": {
                "played": 25,
                "win": 10,
                "draw": 10,
                "lose": 5,
                "goals": {
                  "for": 34,
                  "against": 20
                }
              },
              "home": {
                "played": 13,
                "win": 7,
                "draw": 5,
                "lose": 1,
                "goals": {
                  "for": 25,
                  "against": 9
                }
              },
              "away": {
                "played": 12,
                "win": 3,
                "draw": 5,
                "lose": 4,
                "goals": {
                  "for": 9,
                  "against": 11
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 6,
              "team": {
                "id": 441,
                "name": "Union Santa Fe",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/441.png"
              },
              "points": 39,
              "goalsDiff": 1,
              "group": "Liga Profesional",
              "form": "WLLLW",
              "status": "same",
              "description": null,
              "all": {
                "played": 26,
                "win": 11,
                "draw": 6,
                "lose": 9,
                "goals": {
                  "for": 27,
                  "against": 26
                }
              },
              "home": {
                "played": 14,
                "win": 9,
                "draw": 2,
                "lose": 3,
                "goals": {
                  "for": 18,
                  "against": 8
                }
              },
              "away": {
                "played": 12,
                "win": 2,
                "draw": 4,
                "lose": 6,
                "goals": {
                  "for": 9,
                  "against": 18
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 7,
              "team": {
                "id": 451,
                "name": "Boca Juniors",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/451.png"
              },
              "points": 38,
              "goalsDiff": 6,
              "group": "Liga Profesional",
              "form": "WDWWW",
              "status": "same",
              "description": null,
              "all": {
                "played": 25,
                "win": 10,
                "draw": 8,
                "lose": 7,
                "goals": {
                  "for": 29,
                  "against": 23
                }
              },
              "home": {
                "played": 12,
                "win": 8,
                "draw": 3,
                "lose": 1,
                "goals": {
                  "for": 18,
                  "against": 7
                }
              },
              "away": {
                "played": 13,
                "win": 2,
                "draw": 5,
                "lose": 6,
                "goals": {
                  "for": 11,
                  "against": 16
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 8,
              "team": {
                "id": 455,
                "name": "Atletico Tucuman",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/455.png"
              },
              "points": 37,
              "goalsDiff": 0,
              "group": "Liga Profesional",
              "form": "DLWLW",
              "status": "same",
              "description": null,
              "all": {
                "played": 25,
                "win": 10,
                "draw": 7,
                "lose": 8,
                "goals": {
                  "for": 25,
                  "against": 25
                }
              },
              "home": {
                "played": 13,
                "win": 8,
                "draw": 3,
                "lose": 2,
                "goals": {
                  "for": 18,
                  "against": 11
                }
              },
              "away": {
                "played": 12,
                "win": 2,
                "draw": 4,
                "lose": 6,
                "goals": {
                  "for": 7,
                  "against": 14
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 9,
              "team": {
                "id": 453,
                "name": "Independiente",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/453.png"
              },
              "points": 36,
              "goalsDiff": 7,
              "group": "Liga Profesional",
              "form": "WLWLW",
              "status": "same",
              "description": null,
              "all": {
                "played": 25,
                "win": 8,
                "draw": 12,
                "lose": 5,
                "goals": {
                  "for": 23,
                  "against": 16
                }
              },
              "home": {
                "played": 13,
                "win": 6,
                "draw": 6,
                "lose": 1,
                "goals": {
                  "for": 15,
                  "against": 7
                }
              },
              "away": {
                "played": 12,
                "win": 2,
                "draw": 6,
                "lose": 4,
                "goals": {
                  "for": 8,
                  "against": 9
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 10,
              "team": {
                "id": 478,
                "name": "Instituto Cordoba",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/478.png"
              },
              "points": 36,
              "goalsDiff": 3,
              "group": "Liga Profesional",
              "form": "LWDWL",
              "status": "same",
              "description": null,
              "all": {
                "played": 26,
                "win": 10,
                "draw": 6,
                "lose": 10,
                "goals": {
                  "for": 31,
                  "against": 28
                }
              },
              "home": {
                "played": 12,
                "win": 7,
                "draw": 3,
                "lose": 2,
                "goals": {
                  "for": 19,
                  "against": 9
                }
              },
              "away": {
                "played": 14,
                "win": 3,
                "draw": 3,
                "lose": 8,
                "goals": {
                  "for": 12,
                  "against": 19
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 11,
              "team": {
                "id": 1064,
                "name": "Platense",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/1064.png"
              },
              "points": 36,
              "goalsDiff": 2,
              "group": "Liga Profesional",
              "form": "WLWDD",
              "status": "same",
              "description": null,
              "all": {
                "played": 25,
                "win": 9,
                "draw": 9,
                "lose": 7,
                "goals": {
                  "for": 19,
                  "against": 17
                }
              },
              "home": {
                "played": 12,
                "win": 5,
                "draw": 4,
                "lose": 3,
                "goals": {
                  "for": 7,
                  "against": 6
                }
              },
              "away": {
                "played": 13,
                "win": 4,
                "draw": 5,
                "lose": 4,
                "goals": {
                  "for": 12,
                  "against": 11
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 12,
              "team": {
                "id": 446,
                "name": "Lanus",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/446.png"
              },
              "points": 35,
              "goalsDiff": -3,
              "group": "Liga Profesional",
              "form": "WWDDD",
              "status": "same",
              "description": null,
              "all": {
                "played": 26,
                "win": 8,
                "draw": 11,
                "lose": 7,
                "goals": {
                  "for": 25,
                  "against": 28
                }
              },
              "home": {
                "played": 14,
                "win": 6,
                "draw": 6,
                "lose": 2,
                "goals": {
                  "for": 16,
                  "against": 12
                }
              },
              "away": {
                "played": 12,
                "win": 2,
                "draw": 5,
                "lose": 5,
                "goals": {
                  "for": 9,
                  "against": 16
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 13,
              "team": {
                "id": 450,
                "name": "Estudiantes L.P.",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/450.png"
              },
              "points": 34,
              "goalsDiff": 2,
              "group": "Liga Profesional",
              "form": "WLDDD",
              "status": "same",
              "description": null,
              "all": {
                "played": 25,
                "win": 8,
                "draw": 10,
                "lose": 7,
                "goals": {
                  "for": 32,
                  "against": 30
                }
              },
              "home": {
                "played": 12,
                "win": 4,
                "draw": 6,
                "lose": 2,
                "goals": {
                  "for": 16,
                  "against": 12
                }
              },
              "away": {
                "played": 13,
                "win": 4,
                "draw": 4,
                "lose": 5,
                "goals": {
                  "for": 16,
                  "against": 18
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 14,
              "team": {
                "id": 440,
                "name": "Belgrano Cordoba",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/440.png"
              },
              "points": 34,
              "goalsDiff": 2,
              "group": "Liga Profesional",
              "form": "WLDWD",
              "status": "same",
              "description": null,
              "all": {
                "played": 25,
                "win": 8,
                "draw": 10,
                "lose": 7,
                "goals": {
                  "for": 30,
                  "against": 28
                }
              },
              "home": {
                "played": 13,
                "win": 6,
                "draw": 3,
                "lose": 4,
                "goals": {
                  "for": 18,
                  "against": 15
                }
              },
              "away": {
                "played": 12,
                "win": 2,
                "draw": 7,
                "lose": 3,
                "goals": {
                  "for": 12,
                  "against": 13
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 15,
              "team": {
                "id": 476,
                "name": "Deportivo Riestra",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/476.png"
              },
              "points": 34,
              "goalsDiff": -1,
              "group": "Liga Profesional",
              "form": "DLDDD",
              "status": "same",
              "description": null,
              "all": {
                "played": 26,
                "win": 8,
                "draw": 10,
                "lose": 8,
                "goals": {
                  "for": 26,
                  "against": 27
                }
              },
              "home": {
                "played": 14,
                "win": 8,
                "draw": 5,
                "lose": 1,
                "goals": {
                  "for": 19,
                  "against": 9
                }
              },
              "away": {
                "played": 12,
                "win": 0,
                "draw": 5,
                "lose": 7,
                "goals": {
                  "for": 7,
                  "against": 18
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 16,
              "team": {
                "id": 473,
                "name": "Independ. Rivadavia",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/473.png"
              },
              "points": 34,
              "goalsDiff": -4,
              "group": "Liga Profesional",
              "form": "WWWLD",
              "status": "same",
              "description": null,
              "all": {
                "played": 25,
                "win": 9,
                "draw": 7,
                "lose": 9,
                "goals": {
                  "for": 21,
                  "against": 25
                }
              },
              "home": {
                "played": 12,
                "win": 6,
                "draw": 4,
                "lose": 2,
                "goals": {
                  "for": 11,
                  "against": 8
                }
              },
              "away": {
                "played": 13,
                "win": 3,
                "draw": 3,
                "lose": 7,
                "goals": {
                  "for": 10,
                  "against": 17
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 17,
              "team": {
                "id": 434,
                "name": "Gimnasia L.P.",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/434.png"
              },
              "points": 33,
              "goalsDiff": 0,
              "group": "Liga Profesional",
              "form": "LWLWD",
              "status": "same",
              "description": null,
              "all": {
                "played": 26,
                "win": 8,
                "draw": 9,
                "lose": 9,
                "goals": {
                  "for": 21,
                  "against": 21
                }
              },
              "home": {
                "played": 14,
                "win": 6,
                "draw": 4,
                "lose": 4,
                "goals": {
                  "for": 14,
                  "against": 9
                }
              },
              "away": {
                "played": 12,
                "win": 2,
                "draw": 5,
                "lose": 5,
                "goals": {
                  "for": 7,
                  "against": 12
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 18,
              "team": {
                "id": 452,
                "name": "Tigre",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/452.png"
              },
              "points": 31,
              "goalsDiff": -2,
              "group": "Liga Profesional",
              "form": "DDWLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 25,
                "win": 7,
                "draw": 10,
                "lose": 8,
                "goals": {
                  "for": 26,
                  "against": 28
                }
              },
              "home": {
                "played": 13,
                "win": 3,
                "draw": 6,
                "lose": 4,
                "goals": {
                  "for": 14,
                  "against": 17
                }
              },
              "away": {
                "played": 12,
                "win": 4,
                "draw": 4,
                "lose": 4,
                "goals": {
                  "for": 12,
                  "against": 11
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 19,
              "team": {
                "id": 1065,
                "name": "Central Cordoba de Santiago",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/1065.png"
              },
              "points": 31,
              "goalsDiff": -5,
              "group": "Liga Profesional",
              "form": "WLLWD",
              "status": "same",
              "description": null,
              "all": {
                "played": 26,
                "win": 8,
                "draw": 7,
                "lose": 11,
                "goals": {
                  "for": 29,
                  "against": 34
                }
              },
              "home": {
                "played": 13,
                "win": 5,
                "draw": 2,
                "lose": 6,
                "goals": {
                  "for": 16,
                  "against": 18
                }
              },
              "away": {
                "played": 13,
                "win": 3,
                "draw": 5,
                "lose": 5,
                "goals": {
                  "for": 13,
                  "against": 16
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 20,
              "team": {
                "id": 442,
                "name": "Defensa Y Justicia",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/442.png"
              },
              "points": 31,
              "goalsDiff": -6,
              "group": "Liga Profesional",
              "form": "DWDDW",
              "status": "same",
              "description": null,
              "all": {
                "played": 26,
                "win": 7,
                "draw": 10,
                "lose": 9,
                "goals": {
                  "for": 27,
                  "against": 33
                }
              },
              "home": {
                "played": 12,
                "win": 5,
                "draw": 6,
                "lose": 1,
                "goals": {
                  "for": 13,
                  "against": 10
                }
              },
              "away": {
                "played": 14,
                "win": 2,
                "draw": 4,
                "lose": 8,
                "goals": {
                  "for": 14,
                  "against": 23
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 21,
              "team": {
                "id": 439,
                "name": "Godoy Cruz",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/439.png"
              },
              "points": 30,
              "goalsDiff": -3,
              "group": "Liga Profesional",
              "form": "LDLLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 26,
                "win": 6,
                "draw": 12,
                "lose": 8,
                "goals": {
                  "for": 24,
                  "against": 27
                }
              },
              "home": {
                "played": 14,
                "win": 4,
                "draw": 8,
                "lose": 2,
                "goals": {
                  "for": 15,
                  "against": 10
                }
              },
              "away": {
                "played": 12,
                "win": 2,
                "draw": 4,
                "lose": 6,
                "goals": {
                  "for": 9,
                  "against": 17
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 22,
              "team": {
                "id": 437,
                "name": "Rosario Central",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/437.png"
              },
              "points": 29,
              "goalsDiff": 0,
              "group": "Liga Profesional",
              "form": "LWDLD",
              "status": "same",
              "description": null,
              "all": {
                "played": 25,
                "win": 7,
                "draw": 8,
                "lose": 10,
                "goals": {
                  "for": 25,
                  "against": 25
                }
              },
              "home": {
                "played": 12,
                "win": 4,
                "draw": 4,
                "lose": 4,
                "goals": {
                  "for": 14,
                  "against": 12
                }
              },
              "away": {
                "played": 13,
                "win": 3,
                "draw": 4,
                "lose": 6,
                "goals": {
                  "for": 11,
                  "against": 13
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 23,
              "team": {
                "id": 460,
                "name": "San Lorenzo",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/460.png"
              },
              "points": 29,
              "goalsDiff": -4,
              "group": "Liga Profesional",
              "form": "DLLWD",
              "status": "same",
              "description": null,
              "all": {
                "played": 25,
                "win": 7,
                "draw": 8,
                "lose": 10,
                "goals": {
                  "for": 20,
                  "against": 24
                }
              },
              "home": {
                "played": 12,
                "win": 3,
                "draw": 4,
                "lose": 5,
                "goals": {
                  "for": 10,
                  "against": 12
                }
              },
              "away": {
                "played": 13,
                "win": 4,
                "draw": 4,
                "lose": 5,
                "goals": {
                  "for": 10,
                  "against": 12
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 24,
              "team": {
                "id": 458,
                "name": "Argentinos JRS",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/458.png"
              },
              "points": 26,
              "goalsDiff": -7,
              "group": "Liga Profesional",
              "form": "LLLWL",
              "status": "same",
              "description": null,
              "all": {
                "played": 25,
                "win": 7,
                "draw": 5,
                "lose": 13,
                "goals": {
                  "for": 19,
                  "against": 26
                }
              },
              "home": {
                "played": 13,
                "win": 7,
                "draw": 4,
                "lose": 2,
                "goals": {
                  "for": 16,
                  "against": 7
                }
              },
              "away": {
                "played": 12,
                "win": 0,
                "draw": 1,
                "lose": 11,
                "goals": {
                  "for": 3,
                  "against": 19
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 25,
              "team": {
                "id": 474,
                "name": "Sarmiento Junin",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/474.png"
              },
              "points": 25,
              "goalsDiff": -10,
              "group": "Liga Profesional",
              "form": "DLWLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 26,
                "win": 5,
                "draw": 10,
                "lose": 11,
                "goals": {
                  "for": 17,
                  "against": 27
                }
              },
              "home": {
                "played": 14,
                "win": 4,
                "draw": 6,
                "lose": 4,
                "goals": {
                  "for": 9,
                  "against": 10
                }
              },
              "away": {
                "played": 12,
                "win": 1,
                "draw": 4,
                "lose": 7,
                "goals": {
                  "for": 8,
                  "against": 17
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 26,
              "team": {
                "id": 457,
                "name": "Newells Old Boys",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/457.png"
              },
              "points": 25,
              "goalsDiff": -14,
              "group": "Liga Profesional",
              "form": "DWLLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 25,
                "win": 6,
                "draw": 7,
                "lose": 12,
                "goals": {
                  "for": 19,
                  "against": 33
                }
              },
              "home": {
                "played": 13,
                "win": 5,
                "draw": 3,
                "lose": 5,
                "goals": {
                  "for": 14,
                  "against": 17
                }
              },
              "away": {
                "played": 12,
                "win": 1,
                "draw": 4,
                "lose": 7,
                "goals": {
                  "for": 5,
                  "against": 16
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 27,
              "team": {
                "id": 449,
                "name": "Banfield",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/449.png"
              },
              "points": 24,
              "goalsDiff": -10,
              "group": "Liga Profesional",
              "form": "LDLLD",
              "status": "same",
              "description": null,
              "all": {
                "played": 26,
                "win": 5,
                "draw": 9,
                "lose": 12,
                "goals": {
                  "for": 21,
                  "against": 31
                }
              },
              "home": {
                "played": 12,
                "win": 3,
                "draw": 5,
                "lose": 4,
                "goals": {
                  "for": 14,
                  "against": 13
                }
              },
              "away": {
                "played": 14,
                "win": 2,
                "draw": 4,
                "lose": 8,
                "goals": {
                  "for": 7,
                  "against": 18
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            },
            {
              "rank": 28,
              "team": {
                "id": 2432,
                "name": "Barracas Central",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/2432.png"
              },
              "points": 22,
              "goalsDiff": -18,
              "group": "Liga Profesional",
              "form": "DDWDL",
              "status": "same",
              "description": null,
              "all": {
                "played": 26,
                "win": 4,
                "draw": 10,
                "lose": 12,
                "goals": {
                  "for": 12,
                  "against": 30
                }
              },
              "home": {
                "played": 12,
                "win": 0,
                "draw": 6,
                "lose": 6,
                "goals": {
                  "for": 5,
                  "against": 18
                }
              },
              "away": {
                "played": 14,
                "win": 4,
                "draw": 4,
                "lose": 6,
                "goals": {
                  "for": 7,
                  "against": 12
                }
              },
              "update": "2024-12-08T00:00:00+00:00"
            }
          ]
        ]
      }
    }
  ]
}


export const COPA_LPF_STANDINGS = {
  "get": "standings",
  "parameters": {
    "league": "1032",
    "season": "2024"
  },
  "errors": [],
  "results": 1,
  "paging": {
    "current": 1,
    "total": 1
  },
  "response": [
    {
      "league": {
        "id": 1032,
        "name": "Copa de la Liga Profesional",
        "country": "Argentina",
        "logo": "https:\/\/media.api-sports.io\/football\/leagues\/1032.png",
        "flag": "https:\/\/media.api-sports.io\/flags\/ar.svg",
        "season": 2024,
        "standings": [
          [
            {
              "rank": 1,
              "team": {
                "id": 435,
                "name": "River Plate",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/435.png"
              },
              "points": 27,
              "goalsDiff": 16,
              "group": "Group A",
              "form": "WWLWL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 7,
                "draw": 6,
                "lose": 1,
                "goals": {
                  "for": 26,
                  "against": 10
                }
              },
              "home": {
                "played": 7,
                "win": 4,
                "draw": 3,
                "lose": 0,
                "goals": {
                  "for": 15,
                  "against": 5
                }
              },
              "away": {
                "played": 7,
                "win": 3,
                "draw": 3,
                "lose": 1,
                "goals": {
                  "for": 11,
                  "against": 5
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 2,
              "team": {
                "id": 458,
                "name": "Argentinos JRS",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/458.png"
              },
              "points": 26,
              "goalsDiff": 11,
              "group": "Group A",
              "form": "LLWWL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 7,
                "draw": 5,
                "lose": 2,
                "goals": {
                  "for": 25,
                  "against": 14
                }
              },
              "home": {
                "played": 7,
                "win": 6,
                "draw": 1,
                "lose": 0,
                "goals": {
                  "for": 19,
                  "against": 7
                }
              },
              "away": {
                "played": 7,
                "win": 1,
                "draw": 4,
                "lose": 2,
                "goals": {
                  "for": 6,
                  "against": 7
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 3,
              "team": {
                "id": 2432,
                "name": "Barracas Central",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/2432.png"
              },
              "points": 26,
              "goalsDiff": 5,
              "group": "Group A",
              "form": "LWWWL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 7,
                "draw": 5,
                "lose": 2,
                "goals": {
                  "for": 20,
                  "against": 15
                }
              },
              "home": {
                "played": 7,
                "win": 4,
                "draw": 2,
                "lose": 1,
                "goals": {
                  "for": 11,
                  "against": 8
                }
              },
              "away": {
                "played": 7,
                "win": 3,
                "draw": 3,
                "lose": 1,
                "goals": {
                  "for": 9,
                  "against": 7
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 4,
              "team": {
                "id": 438,
                "name": "Velez Sarsfield",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/438.png"
              },
              "points": 25,
              "goalsDiff": 1,
              "group": "Group A",
              "form": "WLLWW",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 7,
                "draw": 4,
                "lose": 3,
                "goals": {
                  "for": 14,
                  "against": 13
                }
              },
              "home": {
                "played": 7,
                "win": 4,
                "draw": 2,
                "lose": 1,
                "goals": {
                  "for": 8,
                  "against": 4
                }
              },
              "away": {
                "played": 7,
                "win": 3,
                "draw": 2,
                "lose": 2,
                "goals": {
                  "for": 6,
                  "against": 9
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 5,
              "team": {
                "id": 456,
                "name": "Talleres Cordoba",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/456.png"
              },
              "points": 24,
              "goalsDiff": 8,
              "group": "Group A",
              "form": "LLWLW",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 6,
                "draw": 6,
                "lose": 2,
                "goals": {
                  "for": 24,
                  "against": 16
                }
              },
              "home": {
                "played": 7,
                "win": 4,
                "draw": 2,
                "lose": 1,
                "goals": {
                  "for": 14,
                  "against": 7
                }
              },
              "away": {
                "played": 7,
                "win": 2,
                "draw": 4,
                "lose": 1,
                "goals": {
                  "for": 10,
                  "against": 9
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 6,
              "team": {
                "id": 453,
                "name": "Independiente",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/453.png"
              },
              "points": 23,
              "goalsDiff": 4,
              "group": "Group A",
              "form": "LWLLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 6,
                "draw": 5,
                "lose": 3,
                "goals": {
                  "for": 14,
                  "against": 10
                }
              },
              "home": {
                "played": 7,
                "win": 2,
                "draw": 3,
                "lose": 2,
                "goals": {
                  "for": 7,
                  "against": 7
                }
              },
              "away": {
                "played": 7,
                "win": 4,
                "draw": 2,
                "lose": 1,
                "goals": {
                  "for": 7,
                  "against": 3
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 7,
              "team": {
                "id": 478,
                "name": "Instituto Cordoba",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/478.png"
              },
              "points": 17,
              "goalsDiff": 1,
              "group": "Group A",
              "form": "LLLLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 5,
                "draw": 2,
                "lose": 7,
                "goals": {
                  "for": 18,
                  "against": 17
                }
              },
              "home": {
                "played": 8,
                "win": 2,
                "draw": 2,
                "lose": 4,
                "goals": {
                  "for": 9,
                  "against": 11
                }
              },
              "away": {
                "played": 6,
                "win": 3,
                "draw": 0,
                "lose": 3,
                "goals": {
                  "for": 9,
                  "against": 6
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 8,
              "team": {
                "id": 449,
                "name": "Banfield",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/449.png"
              },
              "points": 17,
              "goalsDiff": -1,
              "group": "Group A",
              "form": "WLWLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 4,
                "draw": 5,
                "lose": 5,
                "goals": {
                  "for": 14,
                  "against": 15
                }
              },
              "home": {
                "played": 7,
                "win": 1,
                "draw": 2,
                "lose": 4,
                "goals": {
                  "for": 3,
                  "against": 7
                }
              },
              "away": {
                "played": 7,
                "win": 3,
                "draw": 3,
                "lose": 1,
                "goals": {
                  "for": 11,
                  "against": 8
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 9,
              "team": {
                "id": 445,
                "name": "Huracan",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/445.png"
              },
              "points": 16,
              "goalsDiff": 0,
              "group": "Group A",
              "form": "WLWLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 4,
                "draw": 4,
                "lose": 6,
                "goals": {
                  "for": 12,
                  "against": 12
                }
              },
              "home": {
                "played": 7,
                "win": 3,
                "draw": 3,
                "lose": 1,
                "goals": {
                  "for": 9,
                  "against": 3
                }
              },
              "away": {
                "played": 7,
                "win": 1,
                "draw": 1,
                "lose": 5,
                "goals": {
                  "for": 3,
                  "against": 9
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 10,
              "team": {
                "id": 434,
                "name": "Gimnasia L.P.",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/434.png"
              },
              "points": 16,
              "goalsDiff": -5,
              "group": "Group A",
              "form": "LLWLW",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 5,
                "draw": 1,
                "lose": 8,
                "goals": {
                  "for": 18,
                  "against": 23
                }
              },
              "home": {
                "played": 7,
                "win": 3,
                "draw": 1,
                "lose": 3,
                "goals": {
                  "for": 11,
                  "against": 10
                }
              },
              "away": {
                "played": 7,
                "win": 2,
                "draw": 0,
                "lose": 5,
                "goals": {
                  "for": 7,
                  "against": 13
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 11,
              "team": {
                "id": 437,
                "name": "Rosario Central",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/437.png"
              },
              "points": 14,
              "goalsDiff": -8,
              "group": "Group A",
              "form": "LLLWL",
              "status": "same",
              "description": null,
              "all": {
                "played": 13,
                "win": 4,
                "draw": 2,
                "lose": 7,
                "goals": {
                  "for": 9,
                  "against": 17
                }
              },
              "home": {
                "played": 5,
                "win": 3,
                "draw": 1,
                "lose": 1,
                "goals": {
                  "for": 5,
                  "against": 3
                }
              },
              "away": {
                "played": 8,
                "win": 1,
                "draw": 1,
                "lose": 6,
                "goals": {
                  "for": 4,
                  "against": 14
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 12,
              "team": {
                "id": 476,
                "name": "Deportivo Riestra",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/476.png"
              },
              "points": 12,
              "goalsDiff": -8,
              "group": "Group A",
              "form": "LLWWL",
              "status": "same",
              "description": null,
              "all": {
                "played": 13,
                "win": 3,
                "draw": 3,
                "lose": 7,
                "goals": {
                  "for": 7,
                  "against": 15
                }
              },
              "home": {
                "played": 7,
                "win": 2,
                "draw": 1,
                "lose": 4,
                "goals": {
                  "for": 3,
                  "against": 7
                }
              },
              "away": {
                "played": 6,
                "win": 1,
                "draw": 2,
                "lose": 3,
                "goals": {
                  "for": 4,
                  "against": 8
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 13,
              "team": {
                "id": 455,
                "name": "Atletico Tucuman",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/455.png"
              },
              "points": 10,
              "goalsDiff": -15,
              "group": "Group A",
              "form": "LWLLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 1,
                "draw": 7,
                "lose": 6,
                "goals": {
                  "for": 8,
                  "against": 23
                }
              },
              "home": {
                "played": 7,
                "win": 1,
                "draw": 5,
                "lose": 1,
                "goals": {
                  "for": 6,
                  "against": 8
                }
              },
              "away": {
                "played": 7,
                "win": 0,
                "draw": 2,
                "lose": 5,
                "goals": {
                  "for": 2,
                  "against": 15
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 14,
              "team": {
                "id": 473,
                "name": "Independ. Rivadavia",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/473.png"
              },
              "points": 8,
              "goalsDiff": -12,
              "group": "Group A",
              "form": "LLLLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 2,
                "draw": 2,
                "lose": 10,
                "goals": {
                  "for": 13,
                  "against": 25
                }
              },
              "home": {
                "played": 7,
                "win": 1,
                "draw": 0,
                "lose": 6,
                "goals": {
                  "for": 5,
                  "against": 11
                }
              },
              "away": {
                "played": 7,
                "win": 1,
                "draw": 2,
                "lose": 4,
                "goals": {
                  "for": 8,
                  "against": 14
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            }
          ],
          [
            {
              "rank": 1,
              "team": {
                "id": 439,
                "name": "Godoy Cruz",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/439.png"
              },
              "points": 29,
              "goalsDiff": 10,
              "group": "Group B",
              "form": "LWWLW",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 9,
                "draw": 2,
                "lose": 3,
                "goals": {
                  "for": 16,
                  "against": 6
                }
              },
              "home": {
                "played": 7,
                "win": 4,
                "draw": 2,
                "lose": 1,
                "goals": {
                  "for": 7,
                  "against": 3
                }
              },
              "away": {
                "played": 7,
                "win": 5,
                "draw": 0,
                "lose": 2,
                "goals": {
                  "for": 9,
                  "against": 3
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 2,
              "team": {
                "id": 450,
                "name": "Estudiantes L.P.",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/450.png"
              },
              "points": 27,
              "goalsDiff": 10,
              "group": "Group B",
              "form": "WWWLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 8,
                "draw": 3,
                "lose": 3,
                "goals": {
                  "for": 19,
                  "against": 9
                }
              },
              "home": {
                "played": 7,
                "win": 5,
                "draw": 1,
                "lose": 1,
                "goals": {
                  "for": 12,
                  "against": 2
                }
              },
              "away": {
                "played": 7,
                "win": 3,
                "draw": 2,
                "lose": 2,
                "goals": {
                  "for": 7,
                  "against": 7
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 3,
              "team": {
                "id": 442,
                "name": "Defensa Y Justicia",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/442.png"
              },
              "points": 26,
              "goalsDiff": 4,
              "group": "Group B",
              "form": "WWWLW",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 7,
                "draw": 5,
                "lose": 2,
                "goals": {
                  "for": 17,
                  "against": 13
                }
              },
              "home": {
                "played": 7,
                "win": 4,
                "draw": 2,
                "lose": 1,
                "goals": {
                  "for": 9,
                  "against": 7
                }
              },
              "away": {
                "played": 7,
                "win": 3,
                "draw": 3,
                "lose": 1,
                "goals": {
                  "for": 8,
                  "against": 6
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 4,
              "team": {
                "id": 451,
                "name": "Boca Juniors",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/451.png"
              },
              "points": 25,
              "goalsDiff": 8,
              "group": "Group B",
              "form": "WLWWW",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 7,
                "draw": 4,
                "lose": 3,
                "goals": {
                  "for": 20,
                  "against": 12
                }
              },
              "home": {
                "played": 7,
                "win": 5,
                "draw": 2,
                "lose": 0,
                "goals": {
                  "for": 13,
                  "against": 6
                }
              },
              "away": {
                "played": 7,
                "win": 2,
                "draw": 2,
                "lose": 3,
                "goals": {
                  "for": 7,
                  "against": 6
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 5,
              "team": {
                "id": 436,
                "name": "Racing Club",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/436.png"
              },
              "points": 24,
              "goalsDiff": 13,
              "group": "Group B",
              "form": "WWWLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 7,
                "draw": 3,
                "lose": 4,
                "goals": {
                  "for": 24,
                  "against": 11
                }
              },
              "home": {
                "played": 7,
                "win": 3,
                "draw": 1,
                "lose": 3,
                "goals": {
                  "for": 10,
                  "against": 6
                }
              },
              "away": {
                "played": 7,
                "win": 4,
                "draw": 2,
                "lose": 1,
                "goals": {
                  "for": 14,
                  "against": 5
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 6,
              "team": {
                "id": 446,
                "name": "Lanus",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/446.png"
              },
              "points": 23,
              "goalsDiff": 6,
              "group": "Group B",
              "form": "LLLWW",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 7,
                "draw": 2,
                "lose": 5,
                "goals": {
                  "for": 20,
                  "against": 14
                }
              },
              "home": {
                "played": 7,
                "win": 2,
                "draw": 2,
                "lose": 3,
                "goals": {
                  "for": 9,
                  "against": 9
                }
              },
              "away": {
                "played": 7,
                "win": 5,
                "draw": 0,
                "lose": 2,
                "goals": {
                  "for": 11,
                  "against": 5
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 7,
              "team": {
                "id": 457,
                "name": "Newells Old Boys",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/457.png"
              },
              "points": 21,
              "goalsDiff": -2,
              "group": "Group B",
              "form": "LLWLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 6,
                "draw": 3,
                "lose": 5,
                "goals": {
                  "for": 13,
                  "against": 15
                }
              },
              "home": {
                "played": 7,
                "win": 2,
                "draw": 2,
                "lose": 3,
                "goals": {
                  "for": 5,
                  "against": 10
                }
              },
              "away": {
                "played": 7,
                "win": 4,
                "draw": 1,
                "lose": 2,
                "goals": {
                  "for": 8,
                  "against": 5
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 8,
              "team": {
                "id": 441,
                "name": "Union Santa Fe",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/441.png"
              },
              "points": 20,
              "goalsDiff": 2,
              "group": "Group B",
              "form": "WLLLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 5,
                "draw": 5,
                "lose": 4,
                "goals": {
                  "for": 16,
                  "against": 14
                }
              },
              "home": {
                "played": 7,
                "win": 2,
                "draw": 2,
                "lose": 3,
                "goals": {
                  "for": 9,
                  "against": 9
                }
              },
              "away": {
                "played": 7,
                "win": 3,
                "draw": 3,
                "lose": 1,
                "goals": {
                  "for": 7,
                  "against": 5
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 9,
              "team": {
                "id": 1064,
                "name": "Platense",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/1064.png"
              },
              "points": 18,
              "goalsDiff": -4,
              "group": "Group B",
              "form": "WWLLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 4,
                "draw": 6,
                "lose": 4,
                "goals": {
                  "for": 10,
                  "against": 14
                }
              },
              "home": {
                "played": 7,
                "win": 2,
                "draw": 4,
                "lose": 1,
                "goals": {
                  "for": 6,
                  "against": 4
                }
              },
              "away": {
                "played": 7,
                "win": 2,
                "draw": 2,
                "lose": 3,
                "goals": {
                  "for": 4,
                  "against": 10
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 10,
              "team": {
                "id": 460,
                "name": "San Lorenzo",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/460.png"
              },
              "points": 16,
              "goalsDiff": -4,
              "group": "Group B",
              "form": "LLLWW",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 3,
                "draw": 7,
                "lose": 4,
                "goals": {
                  "for": 10,
                  "against": 14
                }
              },
              "home": {
                "played": 7,
                "win": 3,
                "draw": 2,
                "lose": 2,
                "goals": {
                  "for": 5,
                  "against": 5
                }
              },
              "away": {
                "played": 7,
                "win": 0,
                "draw": 5,
                "lose": 2,
                "goals": {
                  "for": 5,
                  "against": 9
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 11,
              "team": {
                "id": 440,
                "name": "Belgrano Cordoba",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/440.png"
              },
              "points": 14,
              "goalsDiff": -2,
              "group": "Group B",
              "form": "LWWLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 3,
                "draw": 5,
                "lose": 6,
                "goals": {
                  "for": 19,
                  "against": 21
                }
              },
              "home": {
                "played": 7,
                "win": 1,
                "draw": 3,
                "lose": 3,
                "goals": {
                  "for": 8,
                  "against": 11
                }
              },
              "away": {
                "played": 7,
                "win": 2,
                "draw": 2,
                "lose": 3,
                "goals": {
                  "for": 11,
                  "against": 10
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 12,
              "team": {
                "id": 1065,
                "name": "Central Cordoba de Santiago",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/1065.png"
              },
              "points": 11,
              "goalsDiff": -10,
              "group": "Group B",
              "form": "LLLLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 2,
                "draw": 5,
                "lose": 7,
                "goals": {
                  "for": 10,
                  "against": 20
                }
              },
              "home": {
                "played": 7,
                "win": 1,
                "draw": 3,
                "lose": 3,
                "goals": {
                  "for": 6,
                  "against": 8
                }
              },
              "away": {
                "played": 7,
                "win": 1,
                "draw": 2,
                "lose": 4,
                "goals": {
                  "for": 4,
                  "against": 12
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 13,
              "team": {
                "id": 474,
                "name": "Sarmiento Junin",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/474.png"
              },
              "points": 9,
              "goalsDiff": -10,
              "group": "Group B",
              "form": "LLLLW",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 2,
                "draw": 3,
                "lose": 9,
                "goals": {
                  "for": 9,
                  "against": 19
                }
              },
              "home": {
                "played": 7,
                "win": 1,
                "draw": 1,
                "lose": 5,
                "goals": {
                  "for": 6,
                  "against": 9
                }
              },
              "away": {
                "played": 7,
                "win": 1,
                "draw": 2,
                "lose": 4,
                "goals": {
                  "for": 3,
                  "against": 10
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            },
            {
              "rank": 14,
              "team": {
                "id": 452,
                "name": "Tigre",
                "logo": "https:\/\/media.api-sports.io\/football\/teams\/452.png"
              },
              "points": 5,
              "goalsDiff": -18,
              "group": "Group B",
              "form": "LLLLL",
              "status": "same",
              "description": null,
              "all": {
                "played": 14,
                "win": 1,
                "draw": 2,
                "lose": 11,
                "goals": {
                  "for": 7,
                  "against": 25
                }
              },
              "home": {
                "played": 7,
                "win": 1,
                "draw": 1,
                "lose": 5,
                "goals": {
                  "for": 4,
                  "against": 11
                }
              },
              "away": {
                "played": 7,
                "win": 0,
                "draw": 1,
                "lose": 6,
                "goals": {
                  "for": 3,
                  "against": 14
                }
              },
              "update": "2024-04-17T00:00:00+00:00"
            }
          ]
        ]
      }
    }
  ]
}

export const LIVE_EVENTS = [
  {
    "fixture": {
      "id": 1159011,
      "referee": "Yael Falcon Perez, Argentina",
      "timezone": "America/Argentina/Buenos_Aires",
      "date": "2024-12-08T19:15:00-03:00",
      "timestamp": 1733696100,
      "periods": {
        "first": 1733696100,
        "second": null
      },
      "venue": {
        "id": 19570,
        "name": "Estadio Mâs Monumental",
        "city": "Capital Federal, Ciudad de Buenos Aires"
      },
      "status": {
        "long": "First Half",
        "short": "1H",
        "elapsed": 20,
        "extra": null
      }
    },
    "league": {
      "id": 128,
      "name": "Liga Profesional Argentina",
      "country": "Argentina",
      "logo": "https://media.api-sports.io/football/leagues/128.png",
      "flag": "https://media.api-sports.io/flags/ar.svg",
      "season": 2024,
      "round": "2nd Phase - 26"
    },
    "teams": {
      "home": {
        "id": 435,
        "name": "River Plate",
        "logo": "https://media.api-sports.io/football/teams/435.png",
        "winner": null
      },
      "away": {
        "id": 437,
        "name": "Rosario Central",
        "logo": "https://media.api-sports.io/football/teams/437.png",
        "winner": null
      }
    },
    "goals": {
      "home": 0,
      "away": 0
    },
    "score": {
      "halftime": {
        "home": 0,
        "away": 0
      },
      "fulltime": {
        "home": null,
        "away": null
      },
      "extratime": {
        "home": null,
        "away": null
      },
      "penalty": {
        "home": null,
        "away": null
      }
    },
    "events": []
  },
  {
    "fixture": {
      "id": 1159011,
      "referee": "Yael Falcon Perez, Argentina",
      "timezone": "America/Argentina/Buenos_Aires",
      "date": "2024-12-08T19:15:00-03:00",
      "timestamp": 1733696100,
      "periods": {
        "first": 1733696100,
        "second": null
      },
      "venue": {
        "id": 19570,
        "name": "Estadio Mâs Monumental",
        "city": "Capital Federal, Ciudad de Buenos Aires"
      },
      "status": {
        "long": "First Half",
        "short": "1H",
        "elapsed": 20,
        "extra": null
      }
    },
    "league": {
      "id": 128,
      "name": "Liga Profesional Argentina",
      "country": "Argentina",
      "logo": "https://media.api-sports.io/football/leagues/128.png",
      "flag": "https://media.api-sports.io/flags/ar.svg",
      "season": 2024,
      "round": "2nd Phase - 26"
    },
    "teams": {
      "home": {
        "id": 435,
        "name": "River Plate",
        "logo": "https://media.api-sports.io/football/teams/435.png",
        "winner": null
      },
      "away": {
        "id": 437,
        "name": "Rosario Central",
        "logo": "https://media.api-sports.io/football/teams/437.png",
        "winner": null
      }
    },
    "goals": {
      "home": 0,
      "away": 0
    },
    "score": {
      "halftime": {
        "home": 0,
        "away": 0
      },
      "fulltime": {
        "home": null,
        "away": null
      },
      "extratime": {
        "home": null,
        "away": null
      },
      "penalty": {
        "home": null,
        "away": null
      }
    },
    "events": []
  },
  {
    "fixture": {
      "id": 1159011,
      "referee": "Yael Falcon Perez, Argentina",
      "timezone": "America/Argentina/Buenos_Aires",
      "date": "2024-12-08T19:15:00-03:00",
      "timestamp": 1733696100,
      "periods": {
        "first": 1733696100,
        "second": null
      },
      "venue": {
        "id": 19570,
        "name": "Estadio Mâs Monumental",
        "city": "Capital Federal, Ciudad de Buenos Aires"
      },
      "status": {
        "long": "First Half",
        "short": "1H",
        "elapsed": 20,
        "extra": null
      }
    },
    "league": {
      "id": 128,
      "name": "Liga Profesional Argentina",
      "country": "Argentina",
      "logo": "https://media.api-sports.io/football/leagues/128.png",
      "flag": "https://media.api-sports.io/flags/ar.svg",
      "season": 2024,
      "round": "2nd Phase - 26"
    },
    "teams": {
      "home": {
        "id": 435,
        "name": "River Plate",
        "logo": "https://media.api-sports.io/football/teams/435.png",
        "winner": null
      },
      "away": {
        "id": 437,
        "name": "Rosario Central",
        "logo": "https://media.api-sports.io/football/teams/437.png",
        "winner": null
      }
    },
    "goals": {
      "home": 0,
      "away": 0
    },
    "score": {
      "halftime": {
        "home": 0,
        "away": 0
      },
      "fulltime": {
        "home": null,
        "away": null
      },
      "extratime": {
        "home": null,
        "away": null
      },
      "penalty": {
        "home": null,
        "away": null
      }
    },
    "events": []
  },{
    "fixture": {
      "id": 1159011,
      "referee": "Yael Falcon Perez, Argentina",
      "timezone": "America/Argentina/Buenos_Aires",
      "date": "2024-12-08T19:15:00-03:00",
      "timestamp": 1733696100,
      "periods": {
        "first": 1733696100,
        "second": null
      },
      "venue": {
        "id": 19570,
        "name": "Estadio Mâs Monumental",
        "city": "Capital Federal, Ciudad de Buenos Aires"
      },
      "status": {
        "long": "First Half",
        "short": "1H",
        "elapsed": 20,
        "extra": null
      }
    },
    "league": {
      "id": 128,
      "name": "Liga Profesional Argentina",
      "country": "Argentina",
      "logo": "https://media.api-sports.io/football/leagues/128.png",
      "flag": "https://media.api-sports.io/flags/ar.svg",
      "season": 2024,
      "round": "2nd Phase - 26"
    },
    "teams": {
      "home": {
        "id": 435,
        "name": "River Plate",
        "logo": "https://media.api-sports.io/football/teams/435.png",
        "winner": null
      },
      "away": {
        "id": 437,
        "name": "Rosario Central",
        "logo": "https://media.api-sports.io/football/teams/437.png",
        "winner": null
      }
    },
    "goals": {
      "home": 0,
      "away": 0
    },
    "score": {
      "halftime": {
        "home": 0,
        "away": 0
      },
      "fulltime": {
        "home": null,
        "away": null
      },
      "extratime": {
        "home": null,
        "away": null
      },
      "penalty": {
        "home": null,
        "away": null
      }
    },
    "events": []
  },{
    "fixture": {
      "id": 1159011,
      "referee": "Yael Falcon Perez, Argentina",
      "timezone": "America/Argentina/Buenos_Aires",
      "date": "2024-12-08T19:15:00-03:00",
      "timestamp": 1733696100,
      "periods": {
        "first": 1733696100,
        "second": null
      },
      "venue": {
        "id": 19570,
        "name": "Estadio Mâs Monumental",
        "city": "Capital Federal, Ciudad de Buenos Aires"
      },
      "status": {
        "long": "First Half",
        "short": "1H",
        "elapsed": 20,
        "extra": null
      }
    },
    "league": {
      "id": 128,
      "name": "Liga Profesional Argentina",
      "country": "Argentina",
      "logo": "https://media.api-sports.io/football/leagues/128.png",
      "flag": "https://media.api-sports.io/flags/ar.svg",
      "season": 2024,
      "round": "2nd Phase - 26"
    },
    "teams": {
      "home": {
        "id": 435,
        "name": "River Plate",
        "logo": "https://media.api-sports.io/football/teams/435.png",
        "winner": null
      },
      "away": {
        "id": 437,
        "name": "Rosario Central",
        "logo": "https://media.api-sports.io/football/teams/437.png",
        "winner": null
      }
    },
    "goals": {
      "home": 0,
      "away": 0
    },
    "score": {
      "halftime": {
        "home": 0,
        "away": 0
      },
      "fulltime": {
        "home": null,
        "away": null
      },
      "extratime": {
        "home": null,
        "away": null
      },
      "penalty": {
        "home": null,
        "away": null
      }
    },
    "events": []
  },{
    "fixture": {
      "id": 1159011,
      "referee": "Yael Falcon Perez, Argentina",
      "timezone": "America/Argentina/Buenos_Aires",
      "date": "2024-12-08T19:15:00-03:00",
      "timestamp": 1733696100,
      "periods": {
        "first": 1733696100,
        "second": null
      },
      "venue": {
        "id": 19570,
        "name": "Estadio Mâs Monumental",
        "city": "Capital Federal, Ciudad de Buenos Aires"
      },
      "status": {
        "long": "First Half",
        "short": "1H",
        "elapsed": 20,
        "extra": null
      }
    },
    "league": {
      "id": 128,
      "name": "Liga Profesional Argentina",
      "country": "Argentina",
      "logo": "https://media.api-sports.io/football/leagues/128.png",
      "flag": "https://media.api-sports.io/flags/ar.svg",
      "season": 2024,
      "round": "2nd Phase - 26"
    },
    "teams": {
      "home": {
        "id": 435,
        "name": "River Plate",
        "logo": "https://media.api-sports.io/football/teams/435.png",
        "winner": null
      },
      "away": {
        "id": 437,
        "name": "Rosario Central",
        "logo": "https://media.api-sports.io/football/teams/437.png",
        "winner": null
      }
    },
    "goals": {
      "home": 0,
      "away": 0
    },
    "score": {
      "halftime": {
        "home": 0,
        "away": 0
      },
      "fulltime": {
        "home": null,
        "away": null
      },
      "extratime": {
        "home": null,
        "away": null
      },
      "penalty": {
        "home": null,
        "away": null
      }
    },
    "events": []
  },{
    "fixture": {
      "id": 1159011,
      "referee": "Yael Falcon Perez, Argentina",
      "timezone": "America/Argentina/Buenos_Aires",
      "date": "2024-12-08T19:15:00-03:00",
      "timestamp": 1733696100,
      "periods": {
        "first": 1733696100,
        "second": null
      },
      "venue": {
        "id": 19570,
        "name": "Estadio Mâs Monumental",
        "city": "Capital Federal, Ciudad de Buenos Aires"
      },
      "status": {
        "long": "First Half",
        "short": "1H",
        "elapsed": 20,
        "extra": null
      }
    },
    "league": {
      "id": 128,
      "name": "Liga Profesional Argentina",
      "country": "Argentina",
      "logo": "https://media.api-sports.io/football/leagues/128.png",
      "flag": "https://media.api-sports.io/flags/ar.svg",
      "season": 2024,
      "round": "2nd Phase - 26"
    },
    "teams": {
      "home": {
        "id": 435,
        "name": "River Plate",
        "logo": "https://media.api-sports.io/football/teams/435.png",
        "winner": null
      },
      "away": {
        "id": 437,
        "name": "Rosario Central",
        "logo": "https://media.api-sports.io/football/teams/437.png",
        "winner": null
      }
    },
    "goals": {
      "home": 0,
      "away": 0
    },
    "score": {
      "halftime": {
        "home": 0,
        "away": 0
      },
      "fulltime": {
        "home": null,
        "away": null
      },
      "extratime": {
        "home": null,
        "away": null
      },
      "penalty": {
        "home": null,
        "away": null
      }
    },
    "events": []
  },{
    "fixture": {
      "id": 1159011,
      "referee": "Yael Falcon Perez, Argentina",
      "timezone": "America/Argentina/Buenos_Aires",
      "date": "2024-12-08T19:15:00-03:00",
      "timestamp": 1733696100,
      "periods": {
        "first": 1733696100,
        "second": null
      },
      "venue": {
        "id": 19570,
        "name": "Estadio Mâs Monumental",
        "city": "Capital Federal, Ciudad de Buenos Aires"
      },
      "status": {
        "long": "First Half",
        "short": "1H",
        "elapsed": 20,
        "extra": null
      }
    },
    "league": {
      "id": 128,
      "name": "Liga Profesional Argentina",
      "country": "Argentina",
      "logo": "https://media.api-sports.io/football/leagues/128.png",
      "flag": "https://media.api-sports.io/flags/ar.svg",
      "season": 2024,
      "round": "2nd Phase - 26"
    },
    "teams": {
      "home": {
        "id": 435,
        "name": "River Plate",
        "logo": "https://media.api-sports.io/football/teams/435.png",
        "winner": null
      },
      "away": {
        "id": 437,
        "name": "Rosario Central",
        "logo": "https://media.api-sports.io/football/teams/437.png",
        "winner": null
      }
    },
    "goals": {
      "home": 0,
      "away": 0
    },
    "score": {
      "halftime": {
        "home": 0,
        "away": 0
      },
      "fulltime": {
        "home": null,
        "away": null
      },
      "extratime": {
        "home": null,
        "away": null
      },
      "penalty": {
        "home": null,
        "away": null
      }
    },
    "events": []
  },{
    "fixture": {
      "id": 1159011,
      "referee": "Yael Falcon Perez, Argentina",
      "timezone": "America/Argentina/Buenos_Aires",
      "date": "2024-12-08T19:15:00-03:00",
      "timestamp": 1733696100,
      "periods": {
        "first": 1733696100,
        "second": null
      },
      "venue": {
        "id": 19570,
        "name": "Estadio Mâs Monumental",
        "city": "Capital Federal, Ciudad de Buenos Aires"
      },
      "status": {
        "long": "First Half",
        "short": "1H",
        "elapsed": 20,
        "extra": null
      }
    },
    "league": {
      "id": 128,
      "name": "Liga Profesional Argentina",
      "country": "Argentina",
      "logo": "https://media.api-sports.io/football/leagues/128.png",
      "flag": "https://media.api-sports.io/flags/ar.svg",
      "season": 2024,
      "round": "2nd Phase - 26"
    },
    "teams": {
      "home": {
        "id": 435,
        "name": "River Plate",
        "logo": "https://media.api-sports.io/football/teams/435.png",
        "winner": null
      },
      "away": {
        "id": 437,
        "name": "Rosario Central",
        "logo": "https://media.api-sports.io/football/teams/437.png",
        "winner": null
      }
    },
    "goals": {
      "home": 0,
      "away": 0
    },
    "score": {
      "halftime": {
        "home": 0,
        "away": 0
      },
      "fulltime": {
        "home": null,
        "away": null
      },
      "extratime": {
        "home": null,
        "away": null
      },
      "penalty": {
        "home": null,
        "away": null
      }
    },
    "events": []
  },{
    "fixture": {
      "id": 1159011,
      "referee": "Yael Falcon Perez, Argentina",
      "timezone": "America/Argentina/Buenos_Aires",
      "date": "2024-12-08T19:15:00-03:00",
      "timestamp": 1733696100,
      "periods": {
        "first": 1733696100,
        "second": null
      },
      "venue": {
        "id": 19570,
        "name": "Estadio Mâs Monumental",
        "city": "Capital Federal, Ciudad de Buenos Aires"
      },
      "status": {
        "long": "First Half",
        "short": "1H",
        "elapsed": 20,
        "extra": null
      }
    },
    "league": {
      "id": 128,
      "name": "Liga Profesional Argentina",
      "country": "Argentina",
      "logo": "https://media.api-sports.io/football/leagues/128.png",
      "flag": "https://media.api-sports.io/flags/ar.svg",
      "season": 2024,
      "round": "2nd Phase - 26"
    },
    "teams": {
      "home": {
        "id": 435,
        "name": "River Plate",
        "logo": "https://media.api-sports.io/football/teams/435.png",
        "winner": null
      },
      "away": {
        "id": 437,
        "name": "Rosario Central",
        "logo": "https://media.api-sports.io/football/teams/437.png",
        "winner": null
      }
    },
    "goals": {
      "home": 0,
      "away": 0
    },
    "score": {
      "halftime": {
        "home": 0,
        "away": 0
      },
      "fulltime": {
        "home": null,
        "away": null
      },
      "extratime": {
        "home": null,
        "away": null
      },
      "penalty": {
        "home": null,
        "away": null
      }
    },
    "events": []
  },{
    "fixture": {
      "id": 1159011,
      "referee": "Yael Falcon Perez, Argentina",
      "timezone": "America/Argentina/Buenos_Aires",
      "date": "2024-12-08T19:15:00-03:00",
      "timestamp": 1733696100,
      "periods": {
        "first": 1733696100,
        "second": null
      },
      "venue": {
        "id": 19570,
        "name": "Estadio Mâs Monumental",
        "city": "Capital Federal, Ciudad de Buenos Aires"
      },
      "status": {
        "long": "First Half",
        "short": "1H",
        "elapsed": 20,
        "extra": null
      }
    },
    "league": {
      "id": 128,
      "name": "Liga Profesional Argentina",
      "country": "Argentina",
      "logo": "https://media.api-sports.io/football/leagues/128.png",
      "flag": "https://media.api-sports.io/flags/ar.svg",
      "season": 2024,
      "round": "2nd Phase - 26"
    },
    "teams": {
      "home": {
        "id": 435,
        "name": "River Plate",
        "logo": "https://media.api-sports.io/football/teams/435.png",
        "winner": null
      },
      "away": {
        "id": 437,
        "name": "Rosario Central",
        "logo": "https://media.api-sports.io/football/teams/437.png",
        "winner": null
      }
    },
    "goals": {
      "home": 0,
      "away": 0
    },
    "score": {
      "halftime": {
        "home": 0,
        "away": 0
      },
      "fulltime": {
        "home": null,
        "away": null
      },
      "extratime": {
        "home": null,
        "away": null
      },
      "penalty": {
        "home": null,
        "away": null
      }
    },
    "events": []
  },{
    "fixture": {
      "id": 1159011,
      "referee": "Yael Falcon Perez, Argentina",
      "timezone": "America/Argentina/Buenos_Aires",
      "date": "2024-12-08T19:15:00-03:00",
      "timestamp": 1733696100,
      "periods": {
        "first": 1733696100,
        "second": null
      },
      "venue": {
        "id": 19570,
        "name": "Estadio Mâs Monumental",
        "city": "Capital Federal, Ciudad de Buenos Aires"
      },
      "status": {
        "long": "First Half",
        "short": "1H",
        "elapsed": 20,
        "extra": null
      }
    },
    "league": {
      "id": 128,
      "name": "Liga Profesional Argentina",
      "country": "Argentina",
      "logo": "https://media.api-sports.io/football/leagues/128.png",
      "flag": "https://media.api-sports.io/flags/ar.svg",
      "season": 2024,
      "round": "2nd Phase - 26"
    },
    "teams": {
      "home": {
        "id": 435,
        "name": "River Plate",
        "logo": "https://media.api-sports.io/football/teams/435.png",
        "winner": null
      },
      "away": {
        "id": 437,
        "name": "Rosario Central",
        "logo": "https://media.api-sports.io/football/teams/437.png",
        "winner": null
      }
    },
    "goals": {
      "home": 0,
      "away": 0
    },
    "score": {
      "halftime": {
        "home": 0,
        "away": 0
      },
      "fulltime": {
        "home": null,
        "away": null
      },
      "extratime": {
        "home": null,
        "away": null
      },
      "penalty": {
        "home": null,
        "away": null
      }
    },
    "events": []
  },{
    "fixture": {
      "id": 1159011,
      "referee": "Yael Falcon Perez, Argentina",
      "timezone": "America/Argentina/Buenos_Aires",
      "date": "2024-12-08T19:15:00-03:00",
      "timestamp": 1733696100,
      "periods": {
        "first": 1733696100,
        "second": null
      },
      "venue": {
        "id": 19570,
        "name": "Estadio Mâs Monumental",
        "city": "Capital Federal, Ciudad de Buenos Aires"
      },
      "status": {
        "long": "First Half",
        "short": "1H",
        "elapsed": 20,
        "extra": null
      }
    },
    "league": {
      "id": 128,
      "name": "Liga Profesional Argentina",
      "country": "Argentina",
      "logo": "https://media.api-sports.io/football/leagues/128.png",
      "flag": "https://media.api-sports.io/flags/ar.svg",
      "season": 2024,
      "round": "2nd Phase - 26"
    },
    "teams": {
      "home": {
        "id": 435,
        "name": "River Plate",
        "logo": "https://media.api-sports.io/football/teams/435.png",
        "winner": null
      },
      "away": {
        "id": 437,
        "name": "Rosario Central",
        "logo": "https://media.api-sports.io/football/teams/437.png",
        "winner": null
      }
    },
    "goals": {
      "home": 0,
      "away": 0
    },
    "score": {
      "halftime": {
        "home": 0,
        "away": 0
      },
      "fulltime": {
        "home": null,
        "away": null
      },
      "extratime": {
        "home": null,
        "away": null
      },
      "penalty": {
        "home": null,
        "away": null
      }
    },
    "events": []
  },{
    "fixture": {
      "id": 1159011,
      "referee": "Yael Falcon Perez, Argentina",
      "timezone": "America/Argentina/Buenos_Aires",
      "date": "2024-12-08T19:15:00-03:00",
      "timestamp": 1733696100,
      "periods": {
        "first": 1733696100,
        "second": null
      },
      "venue": {
        "id": 19570,
        "name": "Estadio Mâs Monumental",
        "city": "Capital Federal, Ciudad de Buenos Aires"
      },
      "status": {
        "long": "First Half",
        "short": "1H",
        "elapsed": 20,
        "extra": null
      }
    },
    "league": {
      "id": 128,
      "name": "Liga Profesional Argentina",
      "country": "Argentina",
      "logo": "https://media.api-sports.io/football/leagues/128.png",
      "flag": "https://media.api-sports.io/flags/ar.svg",
      "season": 2024,
      "round": "2nd Phase - 26"
    },
    "teams": {
      "home": {
        "id": 435,
        "name": "River Plate",
        "logo": "https://media.api-sports.io/football/teams/435.png",
        "winner": null
      },
      "away": {
        "id": 437,
        "name": "Rosario Central",
        "logo": "https://media.api-sports.io/football/teams/437.png",
        "winner": null
      }
    },
    "goals": {
      "home": 0,
      "away": 0
    },
    "score": {
      "halftime": {
        "home": 0,
        "away": 0
      },
      "fulltime": {
        "home": null,
        "away": null
      },
      "extratime": {
        "home": null,
        "away": null
      },
      "penalty": {
        "home": null,
        "away": null
      }
    },
    "events": []
  },{
    "fixture": {
      "id": 1159011,
      "referee": "Yael Falcon Perez, Argentina",
      "timezone": "America/Argentina/Buenos_Aires",
      "date": "2024-12-08T19:15:00-03:00",
      "timestamp": 1733696100,
      "periods": {
        "first": 1733696100,
        "second": null
      },
      "venue": {
        "id": 19570,
        "name": "Estadio Mâs Monumental",
        "city": "Capital Federal, Ciudad de Buenos Aires"
      },
      "status": {
        "long": "First Half",
        "short": "1H",
        "elapsed": 20,
        "extra": null
      }
    },
    "league": {
      "id": 128,
      "name": "Liga Profesional Argentina",
      "country": "Argentina",
      "logo": "https://media.api-sports.io/football/leagues/128.png",
      "flag": "https://media.api-sports.io/flags/ar.svg",
      "season": 2024,
      "round": "2nd Phase - 26"
    },
    "teams": {
      "home": {
        "id": 435,
        "name": "River Plate",
        "logo": "https://media.api-sports.io/football/teams/435.png",
        "winner": null
      },
      "away": {
        "id": 437,
        "name": "Rosario Central",
        "logo": "https://media.api-sports.io/football/teams/437.png",
        "winner": null
      }
    },
    "goals": {
      "home": 0,
      "away": 0
    },
    "score": {
      "halftime": {
        "home": 0,
        "away": 0
      },
      "fulltime": {
        "home": null,
        "away": null
      },
      "extratime": {
        "home": null,
        "away": null
      },
      "penalty": {
        "home": null,
        "away": null
      }
    },
    "events": []
  },
]

export const PREQUALIFIED_TEAMS = [
  {
    "id": 436,
    "name": "Racing Club",
    "logo": "https:\/\/media.api-sports.io\/football\/teams\/436.png",
    "cup": COPASUDAMERICANA,
  },
  {
    "id": 450,
    "name": "Estudiantes L.P.",
    "logo": "https:\/\/media.api-sports.io\/football\/teams\/450.png",
    cup: COPADELALIGA_ARG
  },
  {
    cup: LPF_ARG,
  },
  {
    cup: COPAARGENTINA,
  }
]